...
public class NoteAccessGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all noteAccesses").get("/api/note-accesses").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new noteAccess")
                        .post("/api/note-accesses")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"role\": \"VIEWER\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_noteAccess_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created noteAccess").get("#{new_noteAccess_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created noteAccess").delete("#{new_noteAccess_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the NoteAccess entity").exec(scn);
